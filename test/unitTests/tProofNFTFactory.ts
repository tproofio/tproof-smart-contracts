    ...
    describe("Operations on NFTs", () => {
      ...
      describe("Set custom image", () => {
        ...
        it("Should fail (not the owner of NFT)", async () => {
          let lastNftId = fromNftNumToTokenId(totalSupply.sub(1).toNumber(), TEST_CHAIN_ID);
          try {
            await NFTFactory.connect(user02).setCustomImage([lastNftId], ["https://test.com/img.png"]);
          } catch (error) {
            expect(error).to.be.instanceOf(Error);
            expect(error).to.match(/Only owner can change image Url/);
            return;
          }
          expect.fail("NFT imageUrl has been changed from a wallet different from the owner");
        });
        ...
        it("Should fail (not the owner of NFT for just one NFT)", async () => {
          ...
          let user02NftMinted = fromNftNumToTokenId(totalSupply.sub(1).toNumber(), TEST_CHAIN_ID);
          try {
            await NFTFactory.connect(user02).setCustomImage(
              [user01NftMinted, user02NftMinted],
              ["https://test.com/img1.png", "https://test.com/img2.png"]
            );
          } catch (error) {
            expect(error).to.be.instanceOf(Error);
            expect(error).to.match(/Only owner can change image Url/);
          ...
          expect.fail("NFT image Url has been changed from a wallet different from the owner");
        });
        ...
        it("Should edit the image Url", async () => {
          let newImageUrl = "https://test.com/imgNew.png";
          let lastNftId = fromNftNumToTokenId(totalSupply.sub(1).toNumber(), TEST_CHAIN_ID);
          await NFTFactory.connect(user01).setCustomImage([lastNftId], [newImageUrl]);
          let customImageUrl = await NFTFactory.customImageUrl(lastNftId);
          expect(customImageUrl).to.be.equal(newImageUrl);
        });

        it("Should edit multiple url images (2)", async () => {
          let newImgUrl01 = "https://test.com/imgNew1.png";
          let newImgUrl02 = "https://test.com/imgNew2.png";
          let lastNftId = fromNftNumToTokenId(totalSupply.sub(1).toNumber(), TEST_CHAIN_ID);
          let beforeLastNftId = lastNftId.sub(1);
          await NFTFactory.connect(user01).setCustomImage([lastNftId, beforeLastNftId], [newImgUrl01, newImgUrl02]);
          let customImageUrl1 = await NFTFactory.customImageUrl(lastNftId);
          let customImageUrl2 = await NFTFactory.customImageUrl(beforeLastNftId);
          expect(customImageUrl1).to.be.equal(newImgUrl01);
          expect(customImageUrl2).to.be.equal(newImgUrl02);
        });

        it("Should throw error (contract paused)", async () => {
          await NFTFactory.pause();
          let newImageUrl = "https://test.com/imgUrlNew.png";
          let lastNftId = fromNftNumToTokenId(totalSupply.sub(1).toNumber(), TEST_CHAIN_ID);
          try {
            await NFTFactory.connect(user01).setCustomImage([lastNftId], [newImageUrl]);
          } catch (error) {
          ...
          expect.fail(null, null, 'Image Url edited with contract paused');
        });
